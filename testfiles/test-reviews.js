...
const testReviews = async () => {
  try {
    ...
    for (const user of allUsers) {
      console.log(
        `User: ${user.name} (${user.email}) - Phone: ${user.phone} - ID: ${user._id}`
      );
    ...
    if (userWithPhone) {
      ...
      console.log(
        `Name: ${userWithPhone.name}, Email: ${userWithPhone.email}, ID: ${userWithPhone._id}`
      );
    ...
    allReviews.forEach((review, index) => {
      console.log(
        `${index + 1}. User: ${review.userId?.name}, Target: ${
          review.targetType
    ...
    if (bhavishwareddyUser) {
      ...
      const token = jwt.default.sign(
        ...
        process.env.JWT_SECRET ||
          "bandiwala123dfghadfsjtktfgdsaDGFHGJHKJJDSAFGHJ",
        { expiresIn: "7d" }
      );
      console.log(
        `Generated token for bhavishwareddy: ${token.substring(0, 20)}...`
      );
      ...
      userReviews.forEach((review, index) => {
        console.log(
          `  ${index + 1}. Target: ${review.targetType}, Rating: ${
            review.rating
      ...
      console.log(
        `curl -X GET "http://localhost:6111/api/reviews/user/my-reviews" -H "Authorization: Bearer ${token}" -H "Content-Type: application/json"`
      );
    }
